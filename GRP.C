...
//==============================================================================================
	...
	while(que_no<=10)
	{
		...
		randomize();
		random_que=rand()%58; //RANDOM QUESTION WILL BE CHOOSEN & SHOWN ON SCREEN. We have 58 question so mod 58 is taken here.
		cnt=0;
		gotoxy(3,3);
		...
		gotoxy(3,4);
		printf("%d.",que_no);
		//This is a logic to skip questions up to that particular random question.
		...
		cnt=0;
		i=3;
		//This logic prints that particular question.
		while(cnt<7)
		{
			fscanf(fp,"%c",&ch);
			printf("%c",ch);
